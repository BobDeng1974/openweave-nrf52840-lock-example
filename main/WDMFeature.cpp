...
void ServiceEchoClient::PlatformEventHandler(const WeaveDeviceEvent * event, intptr_t arg)
{
    ...
    if (event->Type == DeviceEventType::kServiceConnectivityStateChange)
    {
        if (event->ServiceConnectivityChange.Result == kConnectivity_Established)
        {
            NRF_LOG(TAG, "Starting subscription to service");
            SuccessOrExit(err);
        }
...
WEAVE_ERROR WDMFeature::Init(uint32_t intervalMS)
{
    WEAVE_ERROR err;

    err = mSubscriptionEngine.Init(&nl::Weave::Device::gExchangeManager, this, HandleSubscriptionEngineEvent);
    SuccessOrExit(err);
